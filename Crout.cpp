...
int main()
{
	cout<<setw(50)<<"Assignment # 1. Submitted By Aatif Imtiaz Butt"<<endl<<endl;
	
	// Declaration of variables and arrays.
	
	int n=10;
	double A[10-2][10-2], RESISTOR[10][10], ROH[10][10];
	double B[10-2], V[10-2];
/*	
	// Pointer Method to Define a variable memory Array... NOT WORKING AT THE MOMENT...
	int n,m;
	double ** RESISTOR, ** ROH, ** A;
	double * B, * V;
	cout<<"Please Enter Number Of Nodes To Be Solved In Kirchoff's Circuit:"<<endl;
	cin>>n;
	m=n-2;

	RESISTOR = new (nothrow) double * [n];
	for( int i = 0 ; i < n ; i++ )
	RESISTOR[i] = new (nothrow) double [n];

	ROH = new (nothrow) double * [n];
	for( int i = 0 ; i < n ; i++ )
	ROH[i] = new (nothrow) double [n];

	A = new (nothrow) double * [m];
	for( int i = 0 ; i < m ; i++ )
	A[i] = new (nothrow) double [m];

	B = new (nothrow) double [n-2];
	V = new (nothrow) double [n-2];
*/
...
/*
	//This is how we delete the variable allocation of memory for arrays.
	for( int i = 0 ; i < n ; i++ )
	delete [] RESISTOR[i], ROH[i] ;
	for( int i = 0 ; i < m ; i++ )
	delete [] A[i] ;
	delete[] B, V, A, RESISTOR, ROH;
*/
